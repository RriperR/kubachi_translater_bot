...
WELCOME_TEXT = (
    "Добро пожаловать в словарь кубачинского языка.\n\n"
    "Отправьте слово или короткую фразу на русском или кубачинском языке, "
    "а я постараюсь найти перевод.\n\n"
    "Если хотите быстро разобраться в возможностях бота, откройте /info."
)
...
HELP_TEXT = (
    ...
    "⚙️ Настроить режим поиска: /mode\n"
    "👤 Открыть свой профиль: /me\n"
    "🏆 Посмотреть таблицу лучших: /score\n"
...
COMMENT_HINT_TEXT = (
    ...
    "ответьте командой /comment на нужную словарную статью."
)
SUGGEST_PROMPT_TEXT = (
    "Напишите одним сообщением идею, пожелание или описание проблемы.\n\n"
    "Можно предложить новую функцию, пожаловаться на ошибку или подсказать, "
    "как сделать бота удобнее."
)
SUGGEST_CANCELLED_TEXT = "Отправка предложения отменена."
SUGGEST_CANCEL_BUTTON_TEXT = "Отмена"
SUGGEST_SUCCESS_TEXT = "Спасибо! Предложение отправлено."
...
NO_MORE_RESULTS_TEXT = "Больше результатов нет. Можете отправить другой запрос."
PAGINATION_TEXT = "Показать ещё 10 результатов?"
SEARCH_TOO_MANY_TEXT = (
    "Нашлось слишком много вариантов.\n\n"
    "Попробуйте уточнить запрос: добавить ещё одно слово, "
    "написать фразу точнее или переключить режим через /mode."
)
SEARCH_NOT_FOUND_LITE_TEXT = (
    ...
    "• если знаете перевод, добавить его через /add и попасть в таблицу лучших /score"
)
SEARCH_NOT_FOUND_COMPLEX_TEXT = (
    "Пока ничего не нашлось.\n\n"
    "Попробуйте переформулировать запрос, написать другое слово "
    "или сделать запрос короче.\n\n"
    "Если знаете перевод, его можно добавить через /add и попасть в таблицу лучших /score."
)
MODE_PROMPT_TEXT = (
    "Выберите, как искать перевод:\n\n"
    "Точный поиск: лучше подходит для одного слова.\n"
    "Расширенный поиск: лучше подходит для фраз, описаний и запросов с неточным смыслом."
)
MODE_LITE_TEXT = "⚙️ Включен точный поиск. Используйте его, когда ищете отдельное слово."
MODE_COMPLEX_TEXT = (
    "⚙️ Включен расширенный поиск. Он лучше подходит для фраз, описаний и неточных запросов."
)
...
ME_EMPTY_TEXT = (
    "Пока у меня нет данных для профиля.\n\nСделайте хотя бы один поиск и потом снова откройте /me."
)
ME_TITLE_TEXT = "👤 Ваш профиль"
SCORE_EMPTY_CATEGORY_TEXT = "Пока нет данных"
SCORE_INTRO_TEXT = (
    "🏆 Таблица лучших\n\n"
    "По умолчанию имена скрыты. Вы можете сами выбрать, как отображаться в рейтинге."
)
SCORE_SHOW_TELEGRAM_SUCCESS_TEXT = (
    "Готово. Теперь в таблице будет показываться ваш Telegram-ник или имя."
)
SCORE_HIDE_SUCCESS_TEXT = "Готово. Теперь вы снова отображаетесь анонимно."
SCORE_ALIAS_PROMPT_TEXT = (
    "Напишите имя для таблицы лучших.\n\n"
    "Можно использовать 2-24 символа: буквы, цифры, пробел, _, - и точку."
)
SCORE_ALIAS_SUCCESS_TEXT = "Готово. В таблице будет показано имя: {alias}"
SCORE_ALIAS_ERROR_TEXT = "Имя должно быть от 2 до 24 символов, без ссылок, @username и команд."
SCORE_BUTTON_TELEGRAM_TEXT = "Показывать мой Telegram-ник"
...
2. Если ответ не нашёлся, откройте /mode и включите расширенный поиск.
...
5. Если есть идея, пожелание или замечание, используйте /suggest.
6. Если хотите посмотреть свой режим поиска и активность, откройте /me.
7. Если хотите посмотреть таблицу лучших, откройте /score.
...
ADD_GUIDE_TEXT = """Добавим новый перевод по шагам. Понадобятся 4 ответа:

1. Слово на кубачинском. Если есть синонимы, перечислите их через запятую.
2. Перевод на русском.
3. Примеры или фразы с переводом. Если примеров несколько, разделяйте их знаком %.
4. Дополнительная информация. Если записей несколько, разделяйте их знаком \\.

Если какого-то пункта нет, просто отправьте 0."""
...
COMMENT_PROMPT = (
    "Напишите комментарий к этой словарной статье.\n\n"
    "Это может быть уточнение, пояснение или полезное дополнение."
)
...
ADMIN_ROOT_TEXT = """Панель администратора.
...
ADMIN_BROADCAST_TEXT_PROMPT = (
    "Пришлите сообщение для рассылки одним сообщением.\n\n"
    "Поддерживаются:\n"
    "• обычный текст\n"
    "• фото с подписью\n"
    "• альбом из 2-10 изображений\n"
    "• файл с подписью\n\n"
    "После этого я покажу превью и предложу выбрать аудиторию."
)
ADMIN_BROADCAST_EMPTY_TEXT = "Текст рассылки не должен быть пустым."
ADMIN_BROADCAST_UNSUPPORTED_TEXT = (
    "Поддерживаются только текст, фото с подписью, альбом изображений или файл с подписью.\n\n"
    "Отправьте рассылку одним сообщением."
)
ADMIN_BROADCAST_PREVIEW_TITLE = "📨 Превью рассылки"
ADMIN_BROADCAST_AUDIENCE_PROMPT = "Выберите, кому отправить рассылку:"
ADMIN_BROADCAST_DAYS_PROMPT = (
    "Введите число дней.\n\nРассылка уйдёт только тем, кто был активен за этот период."
)
...
ADMIN_ENTRIES_WORD_FILTER_PROMPT = "Введите слово или часть перевода для поиска по статьям."
