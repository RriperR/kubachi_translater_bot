...
### VPS Через SSH-Туннель

Если бот работает на Linux/VPS, а embedding-сервис поднят на другой машине и проброшен на VPS через `ssh -R 127.0.0.1:18080:127.0.0.1:8080`, используй отдельный compose-override:
...
Эта команда:
- поднимает `db`;
- собирает и запускает `bot`;
- переводит `bot` в `host`-network;
- заставляет `bot` ходить в Postgres через `127.0.0.1:5434`;
- заставляет `bot` ходить в embedding-сервис через `http://127.0.0.1:18080`.

Остановить стек можно одной командой:

```bash
make vps-tunnel-down
```

Важно:
- SSH-туннель на `127.0.0.1:18080` должен быть уже поднят;
