...
Схема приложения теперь описана через Alembic. Runtime `ensure_schema()` пока сохранен для обратной совместимости, но каноничный способ поднимать и обновлять схему теперь такой:
